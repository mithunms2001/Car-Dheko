...
def main():
    ...
    set_background_image("C:\\Users\\Home\\Car_dekho\\Car_Dheko\\Car_image.jpg")  # Replace with your image file path

    # Check if the CSV file exists
    csv_path = "C:\\Users\\Home\\Car_dekho\\Car_Dheko\\dropped_car_data_set.csv"
    if not os.path.exists(csv_path):
        st.error(f"File not found: {csv_path}")
        return

    # Load the pre-trained model using pickle
    model_path = "C:\\Users\\Home\\Car_dekho\\Car_Dheko\\gradient_boosting_model.pkl"
    ...
    # Perform scaling for numerical features
    ...
    if st.button('Predict Price'):
        try:
            ...
            # Convert to lakhs (assuming the model predicts price in INR)
            predicted_price_in_lakhs = prediction[0] / 100000
            
            # Display the price in lakhs
            st.markdown(
                f"<h2 style='color:#f7e5ac;'>Estimated Price: ₹{predicted_price_in_lakhs:,.2f} Lakhs</h2>",
                unsafe_allow_html=True
            )
        except Exception as e:
